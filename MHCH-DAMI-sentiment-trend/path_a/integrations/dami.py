#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""DAMI with Path A sentiment-trend input (uses upstream Network + layers)."""
...
from path_a.bootstrap import init_environment

init_environment()

import tf_compat as tf  # noqa: E402
from Network import Network  # noqa: E402
from networks.layers.attention import *  # noqa: F403
from networks.layers.transformer import *  # noqa: F403
from path_a.core.trend_features import (  # noqa: E402
    MODE_FEATURE_NAMES,
    resolve_trend_mode,
    sentiment_input_dim,
)
...
class DAMI(Network):
    def __init__(self, memory=0, vocab=None, config_dict=None, **kwargs):
        ...
        self.trend_features = config_dict.get("trend_features", "baseline")
        self.sentiment_dim = sentiment_input_dim(self.trend_features)
        mode = resolve_trend_mode(self.trend_features)
        feat_names = MODE_FEATURE_NAMES.get(mode, ())
        self.logger.info(
            "Trend features mode: {} (sentiment_dim={}, features={})".format(
                mode, self.sentiment_dim,
                'baseline senti' if mode == 'baseline' else list(feat_names)))
    ...
    def _setup_placeholders(self):
        ...
        # sentiment / sentiment-trend (1 or 5 dims per turn)
        self.input_x3 = tf.compat.v1.placeholder(
            tf.float32, [None, self.dia_max_len, self.sentiment_dim], name="input_x3")
    ...
    def _inference(self):
        ...
        # utterance-level sentiment features (already rank 3: [B, D_len, sentiment_dim])
        self.senti_score = self.input_x3
    ...
    def _compute_loss(self):
        ...
        def nll_loss(probs, labels, epsilon=1e-9, scope=None, nb_classes=2):
            ...
            """
            with tf.name_scope(scope or "log_loss"):
